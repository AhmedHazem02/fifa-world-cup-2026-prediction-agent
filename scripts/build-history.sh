...
# 22
cat > src/agent/predictionAgent.ts <<'EOF'
import { GROUP_FIXTURES } from "../data/fixtures.js";
...
EOF
commit "feat: implement PredictionAgent orchestrator class"
...
async function main(): Promise<void> {
  const agent = new PredictionAgent(true);

  switch (command) {
    case "predict": {
      const matchId = process.argv[3];
      if (!matchId) {
        logger.warn("Usage: npm run predict -- predict <matchId>");
        process.exit(1);
      }
...
export const SYSTEM_PROMPT = `You are a FIFA World Cup 2026 prediction agent.
